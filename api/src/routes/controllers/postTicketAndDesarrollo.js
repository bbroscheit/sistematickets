const { Ticket, User, Desarrollo } = require('../../bd');
const fs = require('fs');
const path = require('path');

const postTicketAndDesarrollo = async (state, worker, subject, detail, answer, userresolved, user, desarrolloId, files) => {
    ...
    try {
        ...
        console.log("desarrolloId", desarrolloId)

        if (desarrolloId) {
            const desarrollo = await Desarrollo.findByPk(desarrolloId);
                if (desarrollo) {
                    await setTicket.setDesarrollo(desarrollo);
                }
            }
...
module.exports = postTicketAndDesarrollo;